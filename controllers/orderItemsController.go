...
func GetAllOrderItem() gin.HandlerFunc {
...
func ItemsByOrder(id string) (OrderItems []primitive.M, err error) {
	...
	groupStage := bson.D{
		{"$group",
			bson.D{
				{"_id", bson.D{
					{"order_id", "$order_id"}, {"table_id", "$table_id"}, {"table_number", "$table_number"}}},
	...
	result, err := orderCollection.Aggregate(
		ctx, mongo.Pipeline{
			matchStage,
			lookupStage,
			unwindStage,
...
func GetOrderItem() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			msg := fmt.Sprintf("Error while fetching order item with id %s", orderItemId)
